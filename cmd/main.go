...
func init() {
	...
	utilruntime.Must(alertsv1.AddToScheme(scheme))
...
func main() {
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "newrelic-kubernetes-operator-v2",
		// LeaderElectionID:       "b3350332.k8s.newrelic.com",
	...
	log := ctrl.Log.WithName("controllers").WithName("AlertsRegistration")

	//policy
	if err := (&controller.AlertPolicyReconciler{
		Client:      mgr.GetClient(),
		Scheme:      mgr.GetScheme(),
		Log:         log.WithName("AlertPolicy"),
	...
	if err = (&controller.NrqlConditionReconciler{
		Client:      mgr.GetClient(),
		Scheme:      mgr.GetScheme(),
		Log:         log.WithName("NrqlCondition"),
		AlertClient: interfaces.InitNewClient,
	}).SetupWithManager(mgr); err != nil {
	...
	//destination
	if err = (&controller.AlertDestinationReconciler{
		Client:      mgr.GetClient(),
		Scheme:      mgr.GetScheme(),
		Log:         log.WithName("AlertDestination"),
		AlertClient: interfaces.InitNewClient,
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "AlertDestination")
		os.Exit(1)
	}
	...
	//workflow
	if err = (&controller.AlertWorkflowReconciler{
		Client:      mgr.GetClient(),
		Scheme:      mgr.GetScheme(),
		Log:         log.WithName("AlertWorkflow"),
		AlertClient: interfaces.InitNewClient,
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "AlertWorkflow")
		os.Exit(1)
	}
	...
	//service level
	if err = (&controller.ServiceLevelReconciler{
		Client:             mgr.GetClient(),
		Scheme:             mgr.GetScheme(),
		Log:                log.WithName("ServiceLevel"),
		ServiceLevelClient: interfaces.InitNewClient,
	}).SetupWithManager(mgr); err != nil {
	...
	if os.Getenv("ENABLE_WEBHOOKS") != "false" {
		if err = (&alertsv1.NrqlCondition{}).SetupWebhookWithManager(mgr); err != nil {
			setupLog.Error(err, "unable to create webhook", "webhook", "NrqlCondition")
			os.Exit(1)
		}
	}
	if os.Getenv("ENABLE_WEBHOOKS") != "false" {
		if err = (&alertsv1.AlertDestination{}).SetupWebhookWithManager(mgr); err != nil {
			setupLog.Error(err, "unable to create webhook", "webhook", "AlertDestination")
			os.Exit(1)
		}
	}
	if os.Getenv("ENABLE_WEBHOOKS") != "false" {
		if err = (&alertsv1.AlertChannel{}).SetupWebhookWithManager(mgr); err != nil {
	...
	if os.Getenv("ENABLE_WEBHOOKS") != "false" {
		if err = (&alertsv1.ServiceLevel{}).SetupWebhookWithManager(mgr); err != nil {
			setupLog.Error(err, "unable to create webhook", "webhook", "ServiceLevel")
			os.Exit(1)
		}
	}
